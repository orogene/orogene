...
pub async fn extract_to_dir<P, R>(tarball: R, dir: P) -> Result<(), TorusError>
where
    P: AsRef<Path>,
    R: AsyncRead + Unpin + Send + Sync,
{
    ...
    smol::unblock(move || mkdirp::mkdirp(&takeme))
        .await
        .map_err(|e| TorusError::ExtractIoError(e, Some(dir.clone())))?;
    ...
    let mut entries = ar
        .clone()
        .entries()
        .map_err(|e| TorusError::ExtractIoError(e, None))?;

    while let Some(file) = entries.next().await {
        let f = file.map_err(|e| TorusError::ExtractIoError(e, None))?;
        let header = f.header();
        let path = dir.join(
            header
                .path()
        ...
        if let async_tar::EntryType::Regular = header.entry_type() {
            ...
            smol::unblock(move || mkdirp::mkdirp(&takeme.parent().unwrap()))
                .await
                .map_err(|e| TorusError::ExtractIoError(e, Some(path.parent().unwrap().into())))?;
            let mut writer = fs::OpenOptions::new()
                .write(true)
                .create(true)
                .open(&path)
                .await
                .map_err(|e| TorusError::ExtractIoError(e, Some(path.clone())))?;
    ...
    let mut reader = ar
        .into_inner()
        .map_err(|_| TorusError::MiscError("Failed to get inner Read".into()))?
        .into_inner()
        .into_inner();
    let mut buf = Vec::new();
    reader
